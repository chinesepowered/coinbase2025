...
class BankInvestigationGame:
    ...
    def get_warning_message(self, agent_id: str, question_type: str) -> str:
        """Generate an appropriate warning message using LLM for other agents."""
        prompt = (
            f"You are an agent in a covert bank investigation game. The player is asking about {question_type}. "
            f"Your current suspicion level is {self.agent_states[agent_id].suspicion_level} out of 10. "
            "Generate a concise, tactical warning message to alert the other agent about potential risk."
        )
        response = llama_llm(prompt)
        try:
            message = response.get("choices", [{}])[0].get("message", {}).get("content", "").strip()
            if message:
                return message
            else:
                raise ValueError("Empty message from LLM")
        except Exception as e:
            print(f"[Debug] Error generating warning message via LLM: {e}")
            if question_type == "embezzlement":
                return "The player is asking about the missing money. Be careful with your responses."
            elif question_type == "investigation":
                return "Watch out - they're investigating pretty hard."
            elif self.agent_states[agent_id].suspicion_level > 7:
                return "I don't trust this player. Be cautious."
